...
class CreateFppProcessTasksTable extends \Illuminate\Database\Migrations\Migration
{
    ...
    public function up()
    {
        ...
        Schema::create($this->dbPrefix.'process_tasks', function (Blueprint $table) {
            ...
            $table->string('name')->comment('流程定义节点名称 ');
            $table->bigInteger('node_id')->comment('流程定义节点id ');
        ...
        Schema::create($this->dbPrefix.'process_task_assignees', function (Blueprint $table) {
            ...
            $table->string('node_approver_id')->comment('节点设计审批uuid ');
            $table->bigInteger('assignee_id')->comment('任务分配人 id ');
            $table->string('assignee')->comment('任务分配人名称 ');
            $table->longText('form_data')->comment('节点表单对应的json数据 ');
            $table->bigInteger('form_data_id')->comment('节点表单对应的表单数据id，自定义表单该项通常为 0 或空 ');
        });
    }
