...
class ProcessDefinitionRepository extends BaseRepository
{

    protected static function buildQueryObj(){
        return NProcessDefinition::query();
    }

    public static function getList($perPage, $condition = [], $keyword = null)
    {
        return static::buildQueryObj()->with(['category'])
    ...
    {
        return static::buildQueryObj()->with(['category','group'])->find($id);
    }
    ...
    public static function select ($keyword = null)
    {
        return static::buildQueryObj()
            ...
            ->orderBy('id', 'desc')
            ->get()
            ->map(function (NProcessDefinition $item) {
                $map = $item->toArray();
                $map['value'] = $item->id;
                $map['text'] = $item->name;
                $map['label'] = $item->name;
    ...
    {
        return static::buildQueryObj()->select('id', 'name', 'order_sort')
            ->get()
            ->map(function (NProcessDefinition $model) {
                $data = $model->toArray();
                return $data;
